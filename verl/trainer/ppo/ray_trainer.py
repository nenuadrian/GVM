        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    uids = {cur_index: str(uuid.uuid4()) for cur_index in set(batch.non_tensor_batch['index'])}
                    # batch.non_tensor_batch['uid'] = np.array([str(uuid.uuid4()) for _ in range(len(batch.batch))],
                    #                                          dtype=object)
                    batch.non_tensor_batch['uid'] = np.array([uids[cur_index] for cur_index in batch.non_tensor_batch['index']],
                                                             dtype=object)
